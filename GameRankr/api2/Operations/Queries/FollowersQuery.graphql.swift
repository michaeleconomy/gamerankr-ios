...
extension Api {
  class FollowersQuery: GraphQLQuery {
    static let operationName: String = "Followers"
    static let operationDocument: ApolloAPI.OperationDocument = .init(
      definition: .init(
        #"query Followers($userId: ID!, $after: String) { user(id: $userId) { __typename followers(first: 30, after: $after) { __typename edges { __typename user: node { __typename ...UserBasic } } pageInfo { __typename endCursor hasNextPage } } } }"#,
    ...
    struct Data: Api.SelectionSet {
      ...
      struct User: Api.SelectionSet {
        ...
        var followers: Followers { __data["followers"] }

        /// User.Followers
        ///
        /// Parent Type: `UserConnection`
        struct Followers: Api.SelectionSet {
          let __data: DataDict
          init(_dataDict: DataDict) { __data = _dataDict }
