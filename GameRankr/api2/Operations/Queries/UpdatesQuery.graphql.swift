...
extension Api {
  class UpdatesQuery: GraphQLQuery {
    static let operationName: String = "Updates"
    static let operationDocument: ApolloAPI.OperationDocument = .init(
      definition: .init(
        #"query Updates($after: String) { updates(first: 30, after: $after) { __typename edges { __typename ranking: node { __typename ...RankingFull } } pageInfo { __typename endCursor hasNextPage } } }"#,
    ...
    struct Data: Api.SelectionSet {
      ...
      static var __selections: [ApolloAPI.Selection] { [
        .field("updates", Updates.self, arguments: [
          "first": 30,
          "after": .variable("after")
        ]),
      ] }

      var updates: Updates { __data["updates"] }

      /// Updates
      ///
      /// Parent Type: `RankingConnection`
      struct Updates: Api.SelectionSet {
        let __data: DataDict
        init(_dataDict: DataDict) { __data = _dataDict }
